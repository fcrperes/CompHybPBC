...
(The main differences with respect to the preparation done for compilation are
...
* Method 2: verify_input_circuit(self)
            ...
            - Additionally, the chosen number of virtual qubits must be k ∈ [1,
            t-1] <----
...
* Method 3: msi_circuit(self, msi_clifford_file_loc, msi_clifford_file_name)
            ...
            gadgetized (adaptive) Clifford circuit.
            This adaptive Clifford circuit with magic state injection is
            exported to a different .qasm file where:
                * a main register (dubbed `q_stab`) with a number of qubits = to
                the number of qubits in the original circuit plus the selected
                number of virtual qubits; <----
                * an auxiliary quantum register is created with the number of
                qubits = to the T-count minus the number of virtual qubits, and
                entitled `q_magic`; <----
...
class MeasurementInputError(Error):
    """Exception raised when the number of measurements is different from 1.
    """
    def __init__(
            self,
            message="Invalid circuit form: nr. of mmts is different from 1!"):
        self.message = message
        super().__init__(self.message)
...
class NonCliffordTGenInputError(Error):
    ...
    ones from the Clifford+T generating set.
    """
    def __init__(
            self,
...
class VirtualQubitError(Error):
    """Exception raised when the requested number of virtual qubits is
    incompatible with the circuit.
    """
    def __init__(self,
                 message="The requested number of virtual qubits is invalid!"):
        self.message = message
        super().__init__(self.message)
...
class QuCirc():
    ...
    def __init__(self, input_file_loc, input_file_name, virtual_qubits):
        ...
        Args:
            ...
            virtual_qubits (int): Number of virtual qubits
        """
    ...
    def verify_input_circuit(self):
        ...
        Raises:
            ...
            VirtualQubitError: Exception raised if the chosen nr of virtual
            qubits is not larger than zero and smaller than the nr. of T gates
        ...
        elif not mmt_count == 1:
            raise MeasurementInputError()
        elif not (len(circuit_registers) == 2 and nr_qregs == 1):
            raise RegisterInputError()
        elif not (self.virtual_qubits < t_count and self.virtual_qubits > 0):
            raise VirtualQubitError()
    ...
    def msi_circuit(self, msi_clifford_file_loc, msi_clifford_file_name):
        ...
        for line in circuit_list:
            if line.startswith('qreg '):
                ...
                circuit_list[index] = line.replace(
                    x[2], f'{q_count + self.virtual_qubits}];\n')
            ...
            if line.startswith('creg '):
                ...
                circuit_list[index] = line.replace(
                    x[2], f'{str(mmt_count + self.virtual_qubits)}];\n')

                circuit_list.insert(
                    index, f'qreg q_magic[{t_count - self.virtual_qubits}];\n')
                circuit_list.insert(
                    index + 2,
                    f'creg c_magic[{t_count - self.virtual_qubits}];\n')
                break
        ...
        for line in circuit_list:
            if line.startswith('t '):
                ...
                if k < self.virtual_qubits:
                    ...
                    circuit_list.insert(
                        index + 1,
                        f'measure q_stab[{k_prime}] -> c_stab[{k_prpr}];\n')
                    circuit_list.insert(
                        index + 2, f'if(c_stab[{k_prpr}]==1) s q_stab[{i}];\n')

                else:
                    k_prime = k - self.virtual_qubits
                    circuit_list[index] = line.replace(
                        ';', f',q_magic[{k_prime}];')
                    circuit_list.insert(
                        index + 1,
                        f'measure q_magic[{k_prime}] -> c_magic[{k_prime}];\n')
                    circuit_list.insert(
                        index + 2,
                        f'if(c_magic[{k_prime}]==1) s q_stab[{i}];\n')
